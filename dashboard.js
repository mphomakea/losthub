...
function handleSaveChanges(event) {
  ...
  const displayNameInput = document.getElementById("newName");
  const imageInput = document.getElementById("image");

  const displayName = displayNameInput.value.trim();
  const image = imageInput.files[0];

  // Check if both name and picture are empty before updating the profile
  if (!displayName) {
...
// Add event listener to the "Edit Profile" button and "Save Changes" button
